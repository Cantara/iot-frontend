...
@Controller
@Path(IotFrontendResource.PATH)
@Api(IotFrontendResource.PATH)
@Produces({"application/json"})
public class IotFrontendResource {
    private final static Logger log = LoggerFactory.getLogger(IotFrontendResource.class);
    ...
    @Configure
    public IotFrontendResource(@Configuration String template, @Configuration String defaultName, CounterService counterService) {
        this.template = template;
        this.defaultName = defaultName;
        this.counterService = counterService;
        log.debug("{} Initialized with {}, {}", IotFrontendResource.class.getSimpleName(), kv("template", template), kv("defaultName", defaultName));
    }
    ...
    public Saying hello(@QueryParam("name") @ApiParam(defaultValue = "Mr. Smith") Optional<String> name) {
        log.trace("{} {} {}", v("method", HttpMethod.GET), v("path", IotFrontendResource.PATH), kv("name", name.orElse("null")));
    ...
    public Saying hello(Planet planet) {
        log.trace("{} {} {}", v("method", HttpMethod.POST), v("path", IotFrontendResource.PATH), fields(planet));
